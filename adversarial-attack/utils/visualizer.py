...
from models.gaze_det.ptgaze.common.camera import Camera
from models.gaze_det.ptgaze.common.face import Face
...
class Visualizer:
    ...
    def draw_bbox(
        self, bbox: np.ndarray, color: Tuple[int, int, int] = (0, 255, 0), lw: int = 1
    ) -> None:
    ...
    def draw_3d_points(
        self, points3d: np.ndarray, color: Tuple[int, int, int] = (255, 0, 255), size=3
    ) -> None:
        assert self.image is not None
    ...
    def draw_model_axes(self, face: Face, length: float, lw: int = 2) -> None:
        ...
        axes3d = (
            ...
            @ Rotation.from_euler("XYZ", [0, np.pi, 0]).as_matrix()
        )
